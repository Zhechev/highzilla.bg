...
class ControllerShippingEcont extends Controller {
	...
	public function index() {
		...
		if (!$this->config->get('econt_updated_aps')) { //for APS
			$this->update_aps();
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->request->post['econt_updated_aps'] = true; //for APS
		...
		$data['entry_to_aps'] = $this->language->get('entry_to_aps');
		$data['help_entry_to_aps'] = $this->language->get('help_entry_to_aps');
		...
		$data['entry_shipping_from'] = $this->language->get('entry_shipping_from');
		$data['entry_applicabl_services'] = $this->language->get('entry_applicabl_services');
		...
		if (isset($this->request->post['econt_office_aps_id'])) {
			$data['econt_office_aps_id'] = $this->request->post['econt_office_aps_id'];
		} else {
			$data['econt_office_aps_id'] = $this->config->get('econt_office_aps_id');
		}
		...
		if (isset($this->request->post['econt_to_aps'])) {
			$data['econt_to_aps'] = $this->request->post['econt_to_aps'];
		} else {
			$data['econt_to_aps'] = $this->config->get('econt_to_aps');
		}
		...
		$office_aps = $this->model_shipping_econt->getOffice($data['econt_office_aps_id']);

		if ($office_aps) {
			$data['econt_office_city_aps_id'] = $office_aps['city_id'];
			$data['econt_office_aps_code'] = $office_aps['office_code'];
		} else {
			$data['econt_office_city_aps_id'] = 0;
			$data['econt_office_aps_code'] = '';
		}

		$data['offices_aps'] = $this->model_shipping_econt->getOfficesByCityId($data['econt_office_city_aps_id'], $this->delivery_type, 1);
	...
	private function validate() {
		...
		if ($this->request->post['econt_shipping_from'] == 'APS' && $this->request->post['econt_cd'] && (!$this->request->post['econt_cd_agreement'] || $this->request->post['econt_cd_agreement'] && empty($this->request->post['econt_cd_agreement_num']))) {
	...
	public function install() {
		...
		@mail('support@extensadev.com', 'Econt Express Shipping Module installed (OpenCart) - LST v2', HTTP_CATALOG . ' - ' . $this->config->get('config_name') . "\r\n" . 'version - ' . VERSION . "\r\n" . 'IP - ' . $this->request->server['REMOTE_ADDR'], 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/plain; charset=UTF-8' . "\r\n" . 'From: ' . $this->config->get('config_owner') . ' <' . $this->config->get('config_email') . '>' . "\r\n");
		...
		$data = array(
			...
			'other_info'        => 'Econt Express Shipping Module installed (OpenCart) - LST v2',
			'product_token'     => 'XVZOV5NRSSYPIFYDOM4J5S28',
		);

		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://ls.dev.extensa.bg/api/collectStats');
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('params' => $data)));
	...
	private function update_aps() { //for APS
		$this->load->model('shipping/econt');

		$this->model_shipping_econt->updateTablesAPS();

		$this->load->model('setting/setting');

		$data = $this->model_setting_setting->getSetting('econt');
		$data['econt_updated_aps'] = TRUE;
