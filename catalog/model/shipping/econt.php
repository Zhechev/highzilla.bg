...
class ModelShippingEcont extends Model {
	...
	function getQuote($address) {
		...
		if ($status) {
			...
			if ($this->config->get('econt_to_door')) {
				$to_door = true;
			}
			if ($this->config->get('econt_to_aps')) {
				$to_aps = true;
			}
			if ($this->config->get('econt_to_office') || (!$to_aps && !$to_door)) {
				$to_office = true;
			}
			...
			if ($to_aps) {
				$quote_data['econt_aps'] = array(
					'code'         => 'econt.econt_aps',
					'title'        => $this->language->get('text_description') . ' - ' . $this->language->get('text_to_aps'),
					'cost'         => 0.00,
					'tax_class_id' => 0,
					'text'         => $this->currency->format(0.00, $this->config->get('config_currency'))
				);
			...
			if (!empty($this->session->data['econt']['city_id']) || !empty($this->session->data['econt']['office_city_id']) || !empty($this->session->data['econt']['office_city_aps_id'])) {
				...
				$receiver_address['office_city_aps_id'] = $this->session->data['econt']['office_city_aps_id'];
				$receiver_address['office_aps_id'] = $this->session->data['econt']['office_aps_id'];
			...
			if (empty($receiver_address['city_id'])) {
				if (!empty($receiver_address['office_city_id'])) {
					...
					$city = $this->getCityByCityId($receiver_address['office_city_aps_id']);
					$receiver_address['post_code'] = $city['post_code'];
					$receiver_address['city'] = $city['name'];
					$receiver_address['city_id'] = $city['city_id'];
				} else {
			...
			if (!empty($receiver_address['city_id'])) {
				...
				if ($this->config->get('econt_shipping_from') == 'APS') {
					$sender_office_aps = $this->getOffice($this->config->get('econt_office_aps_id'));
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					if ($product['length'] <= 60 || $product['width'] <= 60 || $product['height'] <= 60) {
						$size_under_60cm = 1;
					}
				...
				if ($to_office) {
					...
					if ($this->config->get('econt_shipping_from') != 'APS') {
						$tariff_sub_code_preffix = $this->config->get('econt_shipping_from');
					} else {
					...
					} elseif ($weight <= 20 && $tariff_sub_code == 'OFFICE_OFFICE' && $size_under_60cm) {
				...
				if ($to_aps) {
					...
					if (!empty($receiver_address['office_aps_id'])) {
						$receiver_office_aps = $this->getOffice($receiver_address['office_aps_id']);
						$row3['receiver']['office_code'] = $receiver_office_aps['office_code'];

						$receiver_city_aps = $this->getCityByCityId($receiver_office_aps['city_id']);
						$row3['receiver']['city'] = $receiver_city_aps['name'];
						$row3['receiver']['post_code'] = $receiver_city_aps['post_code'];
					} elseif (!empty($receiver_address['office_city_aps_id'])) {
						$receiver_city_aps = $this->getCityByCityId($receiver_address['office_city_aps_id']);
						$row3['receiver']['city'] = $receiver_city_aps['name'];
						$row3['receiver']['post_code'] = $receiver_city_aps['post_code'];
					} else {
						$offices = $this->getOfficesByCityId($receiver_address['city_id'], $this->delivery_type, 1);
						...
						if ($offices) {
							...
							$to_aps = false;
							$method_data['quote']['econt_aps']['text'] = '';
						}
					}
				...
				if ($results && !empty($results->result->e)) {
					foreach ($results->result->e as $result) {
						...
						if (isset($rows_array[$key]) && isset(${'receiver_share_sum_' . $rows_array[$key]})) {
							$method_code = 'econt_' . $rows_array[$key];
						...
						if (!empty($result->error)) {
							...
							if ((float)$this->config->get('econt_total_for_free') && ($total >= $this->config->get('econt_total_for_free')) || (int)$this->config->get('econt_count_for_free') && ($this->cart->countProducts() >= $this->config->get('econt_count_for_free')) || (float)$this->config->get('econt_weight_for_free') && ($weight >= $this->config->get('econt_weight_for_free')) || !$receiver_share_sum && $this->config->get('econt_side') == 'SENDER') {
								...
								$method_data['quote'][$method_code]['text'] = $this->currency->format(0.00, $this->config->get('config_currency'));
								...
								$method_data['quote'][$method_code]['text'] = $this->currency->format($method_data['quote'][$method_code]['cost'],$this->config->get('config_currency'));
							} else {
					...
					if (isset($method_data['quote']['econt_aps'])) {
						$method_data['quote']['econt_aps']['text'] = '';
					}
				}
			} else {
	...
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "econt_customer SET customer_id = '" . (int)$this->customer->getId() . "', shipping_to = '" . $this->db->escape($data['shipping_to']) . "', postcode = '" . $this->db->escape($data['postcode']) . "', city = '" . $this->db->escape($data['city']) . "', quarter = '" . $this->db->escape($data['quarter']) . "', street = '" . $this->db->escape($data['street']) . "', street_num = '" . $this->db->escape($data['street_num']) . "', other = '" . $this->db->escape($data['other']) . "', city_id = '" . (int)$data['city_id'] . "', office_id = '" . (int)$data['office_id'] . "', office_aps_id = '" . (int)$data['office_aps_id'] . "'");
	}
	...
	public function getCitiesWithOffices($delivery_type = '', $aps = 0) {
		if (strtolower($this->config->get('config_language')) == 'bg') {
			$suffix = '';
		} else {
			$suffix = '_en';
	...
	public function getOfficesByCityId($city_id, $delivery_type = '', $aps = 0) {
		if (strtolower($this->config->get('config_language')) == 'bg') {
			$suffix = '';
		} else {
			$suffix = '_en';
		...
		$sql .= " WHERE o.city_id = '" . (int)$city_id . "' AND o.is_machine = '" . (int)$aps . "' GROUP BY o.office_id ORDER BY o.name" . $suffix;
