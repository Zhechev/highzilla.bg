...
class ControllerShippingEcont extends Controller {
	...
	public function index() {
		...
		$data['text_services_aps'] = $this->language->get('text_services_aps');
		$data['text_aps_info'] = $this->language->get('text_aps_info');
		...
		if (isset($this->request->post['shipping_to']) && (($this->request->post['shipping_to'] == 'DOOR' && $this->config->get('econt_to_door')) || ($this->request->post['shipping_to'] == 'OFFICE' && $this->config->get('econt_to_office')) || ($this->request->post['shipping_to'] == 'APS' && $this->config->get('econt_to_aps')))) {
			$data['shipping_to'] = $this->request->post['shipping_to'];
		} elseif (isset($shipping_address['shipping_to']) && (($shipping_address['shipping_to'] == 'DOOR' && $this->config->get('econt_to_door')) || ($shipping_address['shipping_to'] == 'OFFICE' && $this->config->get('econt_to_office')) || ($shipping_address['shipping_to'] == 'APS' && $this->config->get('econt_to_aps')))) {
			$data['shipping_to'] = $shipping_address['shipping_to'];
		} elseif (!$this->config->get('econt_to_door') && $this->config->get('econt_to_office') && $this->config->get('econt_to_aps')) {
			$data['shipping_to'] = 'OFFICE';
		} elseif ($this->config->get('econt_to_door') && !$this->config->get('econt_to_office') && !$this->config->get('econt_to_aps')) {
			$data['shipping_to'] = 'DOOR';
		} elseif ($this->config->get('econt_to_door') && !$this->config->get('econt_to_office') && $this->config->get('econt_to_aps')) {
			$data['shipping_to'] = 'APS';
		} else {
			$data['shipping_to'] = 'OFFICE';
		}
		...
		if (isset($this->request->post['office_city_aps_id'])) {
			$data['office_city_aps_id'] = $this->request->post['office_city_aps_id'];
		} elseif (isset($this->session->data['econt']['office_city_aps_id'])) {
			$data['office_city_aps_id'] = $this->session->data['econt']['office_city_aps_id'];
		} else {
			$data['office_city_aps_id'] = 0;
		}
		...
		if (!$this->config->get('econt_to_door') && !$this->config->get('econt_to_office') && !$this->config->get('econt_to_aps')) {
			$data['to_door'] = true;
		} else {
		...
		$data['to_aps'] = $this->config->get('econt_to_aps');
		...
		$data['cities_aps'] = $this->model_shipping_econt->getCitiesWithOffices($this->delivery_type, 1);
		...
		// APS offices
		if ((!$data['office_city_aps_id'] || !$data['office_aps_code']) && $data['office_aps_id']) {
			$office_aps = $this->model_shipping_econt->getOffice($data['office_aps_id']);

			if ($office_aps) {
				$data['office_city_aps_id'] = $office_aps['city_id'];
				$data['office_aps_code'] = $office_aps['office_code'];
			}
		}

		if (!$data['office_city_aps_id'] && $data['city'] && $data['city_id']) {
			$data['office_city_aps_id'] = $data['city_id'];
		}

		$data['offices_aps'] = array();

		if ($data['office_city_aps_id']) {
			$data['offices_aps'] = $this->model_shipping_econt->getOfficesByCityId($data['office_city_aps_id'], $this->delivery_type, 1);
		}
		...
		if (!empty($this->session->data['shipping_methods']['econt']['quote']['econt_aps']['text'])) {
		...
		if (!version_compare(VERSION, '2.2', '>=')) {
			...
			$results['html'] = $this->load->view('shipping/econt', $data);
			$this->response->setOutput(json_encode($results));
		}
	}
	...
	public function getOfficesByCityId() {
		...
		$results = $this->model_shipping_econt->getOfficesByCityId($city_id, $this->delivery_type, $aps);
